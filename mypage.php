    ...
    <div id="wrap">
        <div id="mypage">
            ...
            <div id="mypage_content">
                ...
                <!-- db -->
                <?php
                    $ordersql="select * from shop_order where m_no=$navm_no";
                    $orderrs=mysqli_query($conn,$ordersql);
                    while($orderrow=mysqli_fetch_array($orderrs)){
                        $o_no=$orderrow["o_no"];
                    ?>
                <table class="mypage_table">
                    ...
                    <tr>
                        ...
                        <td>
                            <?php
                            $sql="select count(*) as cnt from order_detail where c_no=$o_no";
                            $rs=mysqli_query($conn,$sql);
                            $row=mysqli_fetch_array($rs);
                            $sql2="select name from order_detail,prod where c_no=$o_no and order_detail.p_no=prod.no limit 0,1";
                            $rs2=mysqli_query($conn,$sql2);
                            $row2=mysqli_fetch_array($rs2);
                            ?>
                            <?php echo ($row2["name"])?>외 (<?php echo ($row["cnt"]-1)?>)건
                            <br><span style="font-size:.8em;"> ( 총 : <?php echo ($row["cnt"])?>개의 상품 )</span>
                        </td>
                    </tr>
                    <tr>
                        <th>주문자정보</th>
                        <td><?php echo $orderrow["name"]?> 님</td>
                    </tr>
                    <tr>
                        <th>연락처</th>
                        <td><?php echo $orderrow["tel"]?></td>
                    </tr>
                    <tr>
                        <th>배송지</th>
                        <td><?php echo $orderrow["addr"]?></td>
                    </tr>
                    <tr>
                        <th>배송상태</th>
                        <td>
                            ...
                            if($orderrow["step"]=="0"){
                            ?>결제완료<?php
                            }else if($orderrow["step"]=="1"){
                                ?>배송대기<?php
                            }else if($orderrow["step"]=="2"){
                                ?>배송중<?php
                            }else if($orderrow["step"]=="3"){
                                ?>배송완료<?php
                            }?>
                        </td>
                    </tr>
                    <tr>
                        <td colspan="2">
                            <a href="order_detail.php?o_no=<?php echo $o_no?>">상세보기</a>
                        </td>
                    </tr>
                </table>
                <?php } ?>
