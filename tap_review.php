    ...
    <div id="wrap">
        <div id="mypage">
            ...
            <div id="mypage_content">
                ...
                <ul class="review_tap_content">
                    <!-- 리뷰작성 -->
                    <?php
                    $id=$_SESSION["id"];
                    $m_nosql="select * from memb where id='$id'";
                    $m_nors=mysqli_query($conn,$m_nosql);
                    $m_norow=mysqli_fetch_array($m_nors);
                    $m_no=$m_norow["m_no"];
                    ?>
                    <li>
                        <ul class="review_content">
                            <?php
                            $checkresql="select * from order_detail left outer join review on review.o_no=order_detail.c_no and order_detail.m_no=$m_no where review.re_no is null";
                            $checkrs=mysqli_query($conn,$checkresql);
                            $chrowNumb=mysqli_num_rows($checkrs);
                            // echo $chrowNumb;
                            if($chrowNumb==0){
                                ?>
                                    <div align="center" style="padding:20px 0"><b>작성 가능한 상품이 없습니다.</b></div>
                                <?php
                            }else{
                            while($ckrow=mysqli_fetch_array($checkrs)){
                                $no=$ckrow["no"];
                                // echo $no;
                                $sql="select * from order_detail where no=$no";
                                $rs=mysqli_query($conn,$sql);
                                $row=mysqli_fetch_array($rs);
                                $p_no=$row["p_no"];
                                $c_no=$row["c_no"];
                                $sql2="select * from prod where no=$p_no";
                                $rs2=mysqli_query($conn,$sql2);
                                $row2=mysqli_fetch_array($rs2);
                                $fname="/file/".$row2["img"];
                                $sql3="select * from shop_order where o_no=$c_no";
                            ...
                            <li>
                                ...
                                <table class="review_content_table">
                                    <tr style="border-bottom: 1px solid #cec6bd;">
                                        <th>구매일</th>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <span><?php echo $row2["smell"]?></span>
                                        </td>
                                    </tr>
                                    <tr>
                                        <td>₩ <?php echo $row2["price"]?></td>
                                        <td>
                                            <input type="button"onclick="location.href='write_review.php?no=<?php echo $no?>'" value="리뷰작성하기" class="go_to_write_review">
                                        </td>
                                    </tr>
                                </table>
                            </li>
                    ...
                    <li>
                        ...
                        $reviewoksql="select * from prod,review where m_no=$m_no and review.p_no=prod.no";
                        $reviewrs=mysqli_query($conn,$reviewoksql);
                        ?>
                        <ul class="review_content">
                            <?php
                                while($row=mysqli_fetch_array($reviewrs)){
                                    $fname="/file/".$row["img"];
                                    $ooo_no=$row["o_no"];
                                    $ppp_no=$row["p_no"];
                                    $re_no=$row["re_no"];
                            ...
                            <li>
                                ...
                                <table class="review_content_table">
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <input type="button" value="리뷰수정하기" onclick="location.href='rewrite_review.php?p_no=<?php echo $ppp_no?>&no=<?php echo $ooo_no?>&re_no=<?php echo $re_no?>&content=<?php echo $ccontent?>'" class="go_to_write_review">
                                        </td>
                                    </tr>
                                    <tr>
                                        <th>별점</th>
