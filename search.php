 ...
 <!-- 헤더 시작 -->
    ...
    <div id="wrap">
        ...
        <div id="search_poster">
            <?php
            $search=$_POST["search"];
            $searchsql="select * from prod where name like '%$search%' OR keyword like '%$search%' or smell like '%$search%' or content like '%$search' or howtouse like '%$search%' or madeby like '%$search%' or size like '%$search%'";
            $searchrs=mysqli_query($conn,$searchsql);
            $searchnumb=mysqli_num_rows($searchrs);
            ?>
            <div id="search_content">
                검색하신 
                <span>'<?php echo $search?>'</span>의<br>
                관련 상품은
                <span> <?php echo $searchnumb?></span>건 입니다.
            </div>
        </div>
        <div id="gift_wrap">
            <!-- 검색하기 while 시작-->
            ...
            if($searchnumb==0){
                ?>
                <div align="center">일치하는 상품이 없습니다.</div>
                <?php
            }else{
            ?>
            <ul class="gift_db">
                <?php 
                    while($SRrow=mysqli_fetch_array($searchrs)){
                    $srimg="/file/".$SRrow["img"];
                    $n0=$SRrow["no"];
                    ?>
                    
                <li onclick="location.href='prod_detail.php?no=<?php echo $n0?>'">
                    <div class="gift_db_img">
                        <img src="<?php echo $srimg?>">
                    </div>
                    <table class="gift_db_text">
                        <tr>
                            <td>
                                <?php echo $SRrow["size"]?><br>
                                <span style="font-size:.8em"><?php echo $SRrow["smell"]?></span>
                            </td>
                        </tr>
                        <tr>
                            <th>
