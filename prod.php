        ...
        <div id="prod">
            ...
            <div id="prod_content">
                ...
                <ul id="prod_list">
                    ...
                    if($kinds=="1"){
                        $pagesql="select count(*) as cnt from prod where kinds='4' and poster=0 or kinds='2' and poster=0 or kinds='3' and poster=0";
                    }else{
                    $pagesql="select count(*) as cnt from prod where kinds='$kinds' and poster=0";
                    }
                    $pagers=mysqli_query($conn,$pagesql);
                    $pagerow=mysqli_fetch_array($pagers);
                    $pageCnt=$pagerow["cnt"];
                    $pageCount=ceil($pageCnt/8);
                    if($page==1){
                    $startRow=($page-1)*8;
                    $midRow=$startRow+4;
                    }else{
                    ...
                    $groupCount=($pageCount/5);
                    $startPage=($group-1)*5+1;
                    $endPage=($startPage+4);
                    // 상품 db sql
                    if($kinds=="1"){
                        $sql="select * from prod where kinds='4' and poster=0 or kinds='2' and poster=0 or kinds='3' and poster=0 order by no desc limit $startRow,$midRow";
                    }else{
                    $sql="select * from prod where kinds='$kinds' and poster=0 order by no desc limit $startRow,$midRow";
                    }
                    $rs=mysqli_query($conn,$sql);
                    ...
                    <?php
                        if($kinds=="1"){
                            $sql2="select * from prod where kinds='4' or kinds='2' or kinds='3' and poster=1 order by no desc limit 0,1";
                        }else{
                        $sql2= "select * from prod where kinds='$kinds' and poster=1 order by no desc limit 0,1";}
                        $rs2=mysqli_query($conn,$sql2);
                        $row2=mysqli_fetch_array($rs2);
                        if(isset($row2["img"])){
                        $fname2="/file/".$row2["img"];
                    ?>
                    <li class="prod_poster" style="border:none">
                        <div id="prod_poster">
                    ...
                    if($kinds=="1"){
                        $sql3="select * from prod where kinds='4' and poster=0 or kinds='2' and poster=0 or kinds='3' and poster=0 order by no desc limit $midRow2,$endRoww";
                    }else{
                        $sql3="select * from prod where kinds='$kinds' and poster=0 order by no desc limit $midRow2,$endRoww";
                    }
            ...
            <div id="page">
                <?php 
                    ...
                    ?>
                    <a href="prod.php?page=<?php echo $prevPage ?>&kinds=<?php echo $kinds?>&title=<?php echo $title?>">[ Prev ]</a>
                    <?php };
                    for($i=$startPage;$i<=$endPage;$i++){
                        if($i>$pageCount)break;
                    ?>
                        <a href="prod.php?page=<?php echo $i ?>&kinds=<?php echo $kinds?>&title=<?php echo $title?>">[ <?php echo $i?> ]</a>
                <?php } ; 
                    if($group<$groupCount){
                        $nextPage=$group*5+1;
                    ?>
                    <a href="prod.php?page=<?php echo $nextPage ?>&kinds=<?php echo $kinds?>&title=<?php echo $title?>">[ Next ]</a>
                    <?php } ?>
            </div>
            <!-- 페이지 끝 -->
        </div>
